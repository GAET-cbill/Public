...
'@
    ...
    if ($nonAdminLoggedOn -eq $false) {
        ...
        $starturl = "https://github.com/GAET-cbill/Public/raw/main/Bloatware/start2.bin"
...
if ($manufacturer -like "*HP*") {
    ...
    if (test-path -Path 'C:\Program Files (x86)\InstallShield Installation Information\{6468C4A5-E47E-405F-B675-A70A70983EA6}\setup.exe') {
        invoke-webrequest -uri "https://github.com/GAET-cbill/Public/raw/main/Bloatware/HPConnOpt.iss" -outfile "C:\Windows\Temp\HPConnOpt.iss"
...
$xml | Out-File -FilePath "$($env:ProgramData)\Debloat\o365.xml"

# Download the ODT
	$odturl = "https://github.com/GAET-cbill/Public/raw/main/Bloatware/odt.exe"
	$odtdestination = "$($env:ProgramData)\Debloat\odt.exe"
	Invoke-WebRequest -Uri $odturl -OutFile $odtdestination -Method Get -UseBasicParsing

# Run it

	Try {
		Start-Process -FilePath "$($env:ProgramData)\Debloat\odt.exe" -ArgumentList "/configure $($env:ProgramData)\Debloat\o365.xml" -Wait -ErrorAction Stop
		Write-Output "Successfully uninstalled: Office 365 Retail"
	}
	
	catch {
    Write-Output "Failed to uninstall: Office 365 Retail"
	}


############################################################################################################
#                                        Remove Any other installed bloat                                  #
#                                                                                                          #
############################################################################################################
